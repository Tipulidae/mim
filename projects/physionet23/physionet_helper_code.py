...
def load_challenge_data(data_folder, patient_id):
    ...
    patient_metadata_file = os.path.join(
        data_folder, patient_id, patient_id + '.txt')
    recording_metadata_file = os.path.join(
        data_folder, patient_id, patient_id + '.tsv')
    ...
    for recording_id in recording_ids:
        if recording_id != 'nan':
            recording_location = os.path.join(
                data_folder, patient_id, recording_id)
...
def load_recording(record_name):
    ...
    with open(header_file, 'r') as f:
        header = [line.strip() for line in f.readlines() if line.strip()]
    ...
    # WFDB format  allows for multiple signal files, but we have not
    ...
    if num_signal_files != 1:
        raise NotImplementedError(
            ...
            f'signal files; one signal file expected.')
    ...
    # Check that the dimensions of the signal data in the signal file is
    # consistent with the dimensions for the signal data given
    # in the header file.
    num_channels = len(channels)
    if np.shape(data) != (num_channels, num_samples):
        raise ValueError('The header file {}'.format(header_file) +
                         ' is inconsistent with the dimensions of the signal '
                         'file.')

    # Check that the initial value and checksums for the signal data in the
    # signal file are consistent with the initial value and
    # checksums for the signal data given in the header file.
    for i in range(num_channels):
        if data[i, 0] != initial_values[i]:
            raise ValueError(f'The initial value in header file {header_file} '
                             f'is inconsistent with the initial value for '
                             f'channel {channels[i]}')
        if np.sum(data[i, :]) != checksums[i]:
            raise ValueError(f'The checksum in header file {header_file} '
                             f'is inconsistent with the initial value for '
                             f'channel {channels[i]}')
...
def get_outcome(string):
    ...
    if variable is None or is_nan(variable):
        raise ValueError('No outcome available. Is your code trying to load '
                         'labels from the hidden data?')
    if variable == 'Good':
        variable = 0
    elif variable == 'Poor':
        variable = 1
...
def get_outcome_probability(string):
    variable = sanitize_scalar_value(
        get_variable(string, 'Outcome probability', str))
    if variable is None or is_nan(variable):
        raise ValueError('No outcome available. Is your code trying to load '
                         'labels from the hidden data?')
    return variable
...
def get_cpc(string):
    ...
    if variable is None or is_nan(variable):
        raise ValueError('No CPC score available. Is your code trying to '
                         'load labels from the hidden data?')
    return variable
...
# Load the Challenge labels for all files in a folder.
...
def save_challenge_outputs(filename, patient_id, outcome,
                           outcome_probability, cpc):
...
# Load the Challenge outputs for all files in folder.
def load_challenge_outputs(folder, patient_ids):
    ...
    for i in range(num_patients):
        output_file = os.path.join(
            folder, patient_ids[i], patient_ids[i] + '.txt')
...
# Remove any quotes, brackets (for singleton arrays), and/or invisible
# characters.
...
def sanitize_boolean_value(x):
    ...
    if (is_finite_number(x) and float(x) == 0) or \
            (x in ('False', 'false', 'F', 'f')):
        return 0
    elif (is_finite_number(x) and float(x) == 1) or \
            (x in ('True', 'true', 'T', 't')):
        return 1
    else:
        return float('nan')
